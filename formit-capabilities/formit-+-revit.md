...
### **Усовершенствования в FormIt 2023.1** <a href="#formit-20221-improvements" id="formit-20221-improvements"></a>

* Местоположение, заданное в Revit, теперь синхронизируется с FormIt при запуске с помощью кнопки «3D-эскиз».
* Улучшена синхронизация линий DWG при запуске FormIt с помощью кнопки «3D-эскиз».
* При необходимости можно выполнить синхронизацию камеры между FormIt и Revit при работе в 3D-эскизе или редактировании связанного объекта FormIt.

### Возможности создания связей в версии 2023 <a href="#formit-20221-improvements" id="formit-20221-improvements"></a>

* Модель FormIt (в формате AXM) можно привязать к Revit с помощью кнопки _**Связь САПР**_ на вкладке «Вставка» в Revit.
* Выбрав связанный элемент FormIt в Revit, его можно открыть в FormIt для оперативного редактирования с помощью новой кнопки _**Редактировать в FormIt**_.
* Используя новый _**столбец «Отправить в Revit»**_ на палитре слоев, можно управлять тем, какие слои отправляются в Revit при наличии связи между эскизом FormIt и Revit. 
...
* Исправлено несколько ошибок, из-за которых преобразование геометрии FormIt из Revit приводило к сбою или зависанию Revit.
...
* Используйте _**режим предварительного просмотра Revit**_ для просмотра геометрии, отправляемой в Revit, чтобы убедиться в корректности проекта, к которому предоставляется общий доступ для последующего детального проектирования.
...
#### Диалоговое окно «Открыть FormIt» для 3D-эскиза

* Нажмите кнопку _**3D-эскиз**_, чтобы запустить FormIt. Укажите объекты, которые будут перенесены в FormIt: все видимые или только выбранные. 
  * Вариант переноса только выбранных объектов удобен, если модель Revit имеет большой размер, а проект будет ссылаться только на часть модели Revit. 
  * Кроме того, можно использовать секущую рамку в Revit, чтобы изолировать внутреннюю область модели Revit и перенести ее в FormIt.

![](../.gitbook/assets/revit-objects.png)

#### 3D-эскиз и уровни

* Если выбран вариант «Все видимые объекты Revit», все видимые уровни Revit становятся уровнями FormIt, которые можно [просмотреть на палитре уровней](../tool-library/levels-and-area.md#creating-and-configuring-levels) или [применить к объектам с помощью палитры свойств](../tool-library/levels-and-area.md#applying-levels).
* Если выбран вариант «Выбранные объекты Revit», то только выбранные уровни станут уровнями FormIt.

#### После запуска FormIt
...
* Примените уровни к объектам FormIt, используя [палитру уровней](../tool-library/levels-and-area.md#applying-levels).

### Рабочий процесс «Отправить в Revit»

* Используйте _**столбец «Отправить в Revit»**_ на палитре слоев, чтобы включить или исключить геометрию для отправки в Revit.
...
* Перенесите геометрию FormIt в Revit с помощью кнопки _**Отправить в Revit**_ на панели инструментов FormIt.
  * Появится запрос на сохранение файла, и модель будет привязана к Revit.
    * **Примечание.** Рекомендуется сохранить файл локально, так как при сохранении файла в Autodesk Docs связь между FormIt и Revit разрывается. Вместо этого используйте [Autodesk Desktop Connector](https://help.autodesk.com/view/CONNECT/RUS/) для предоставления общего доступа к моделям в облаке.
...
* Слои и материалы FormIt переносятся в Revit вместе с геометрией.
  ...
  * Для отправки уровней в Revit необходимо импортировать файл AXM в диалоговом окне «Импорт САПР» и установить флажок «Импорт уровней» в нижней части окна.

### Кнопка «Редактировать в FormIt»

* После связывания модели FormIt в Revit или создания новой модели FormIt с помощью _**3D-эскиза**_ и ее сохранения элемент FormIt сохраняется в Revit, и его можно будет отредактировать позднее.
* Выберите элемент FormIt в Revit, а затем нажмите кнопку _**Редактировать в FormIt**_ на вкладке «Изменить», чтобы запустить модель в FormIt.

![](<../.gitbook/assets/20220809 FormIt-Revit Edit in FormIt.png>)

* См. [учебное пособие по рабочему процессу FormIt — Revit](../formit-primer/part-ii/2.8-advanced-revit-workflows.md) в руководстве FormIt Primer.

### **Импорт и связывание моделей FormIt: совместная работа над проектами в FormIt и Revit** <a href="#importing-formit-models-design-collaboration-between-formit-and-revit" id="importing-formit-models-design-collaboration-between-formit-and-revit"></a>

#### **Предлагаемые рабочие процессы**

* Предоставьте доступ к модели FormIt коллегам, которые затем смогут импортировать модель FormIt в свой проект Revit для использования в качестве справки, а также для визуализации и детализации.
...
* Укажите файл FormIt, который требуется импортировать в текущий проект Revit. Откройте инструмент _**Импорт САПР**_ в Revit. Выберите AXM в списке доступных типов файлов. Найдите файл AXM и нажмите кнопку _Открыть_.
* Материалы FormIt переносятся в Revit и отображаются в диалоговом окне «Материалы» в Revit. Отфильтруйте список для отображения материалов FormIt.
  * Материалы, которые были масштабированы или повернуты в FormIt, сохраняют заданный масштаб и угол поворота в Revit.
...
* При необходимости используйте [встроенные средства диагностики и исправления геометрии](https://formit.autodesk.com/blog/post/using-formit-to-get-sketchup-data-into-revit), чтобы сделать ее сплошной и более подходящей для последующих операций.
...
Программы FormIt и Revit 2022 или более поздней версии обеспечивают встроенную интеграцию, не требующую скачивания дополнительных компонентов.
...
Вместо кнопки _Импорт из FormIt в RVT_ в составе надстройки теперь доступна возможность импорта файлов FormIt (AXM) с помощью инструмента «Импорт САПР» на вкладке «Вставка» в Revit. 
...
* В рамках рабочих процессов, где используются инструменты «Формообразующие элементы-перекрытия» и «По грани» (инструменты конструктора зданий), сначала импортируйте модель FormIt в семейство формообразующих элементов или обобщенных моделей Revit, а затем загрузите ее в среду проекта.
...
## Важные примечания <a href="#revit-important-notes" id="revit-important-notes"></a>

### **Получение поддержки** <a href="#get-support" id="get-support"></a>

Требуется помощь при работе с FormIt и Revit? [Напишите нам на форумах](https://forums.autodesk.com/t5/formit-forum/bd-p/142).
