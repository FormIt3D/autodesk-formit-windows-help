...
### **FormIt 2023.1 改进** <a href="#formit-20221-improvements" id="formit-20221-improvements"></a>

* 通过“三维草图”按钮启动 FormIt 时，在 Revit 中设置的位置现在会同步到 FormIt。
* 改进了通过“三维草图”按钮启动 FormIt 时，DWG 线条的同步。
* 在三维草图中或编辑链接的 FormIt 对象时，在 FormIt 和 Revit 之间可选择同步相机。
...
* 使用剖面框定义 Revit 模型的内部部分，然后单击 _**“三维草图”**_ 按钮以打开 FormIt 并浏览内部布局或定制设计功能。只需单击一下，即可将新设计发送到 Revit。
...
* 确保您已使用 Autodesk 凭据登录到 Revit。启动 _**“三维草图”**_ 按钮时，可能会提示您重新登录到 Autodesk 帐户。这是为了检查您是否有正确的权利。
...
* 如果选择“所有可见 Revit 对象”，则任何可见的 Revit 标高都会变为 FormIt 标高，这些标高可以[在“标高”选项板中查看](../tool-library/levels-and-area.md#creating-and-configuring-levels)或[通过“特性”选项板应用于对象](../tool-library/levels-and-area.md#applying-levels)。
* 如果选择“选定的 Revit 对象”，则只有选定的标高会变为 FormIt 标高。

#### FormIt 启动后

* FormIt 启动后，包含的 Revit 几何图形会在 FormIt 中显示在其自己的图层上。该图层称为“Revit 上下文”，默认情况下处于锁定状态。
* 通过添加形状或应用材质、内容等，以在 FormIt 中建模。
* 从[“标高”选项板](../tool-library/levels-and-area.md#applying-levels)，将标高应用于 FormIt 对象。
...
* FormIt 图层和材质会随几何图形一起传输到 Revit。
  ...
  * 要将标高发送到 Revit，您需要通过“输入 CAD”对话框输入 AXM，并选中底部的“输入标高”。
...
* 请参见 FormIt Primer，以了解 [FormIt-Revit 工作流教程](../formit-primer/part-ii/2.8-advanced-revit-workflows.md)。
...
![在 Revit 中输入 CAD](<../.gitbook/assets/revit-workflows (2).png>)
...
* 要了解更多信息，请参见 [Revit 文档](https://help.autodesk.com/view/RVT/2023/ENU/?guid=GUID-A5F4B6C1-94CD-4433-A4E2-6B39B4BA8E6D)。
...
需要 FormIt + Revit 的帮助？[在论坛上告知我们](https://forums.autodesk.com/t5/formit-forum/bd-p/142)。

下载旧版 Revit 附加模块\
（2021 版或更早版本） <a href="#revit-download" id="revit-download"></a>
------------------------------------------------------------------
