...
O [FormIt 2022.1](https://formit.autodesk.com/blog/post/introducing-formit-2022-1) inclui a capacidade de usar [cotas familiares do FormIt como entradas](https://formit.autodesk.com/page/formit-dynamo#dynamo-input-nodes), apresenta [opções no nível de objeto](https://formit.autodesk.com/page/formit-dynamo#dynamo-formit-options-nodes) e oferece uma visualização prévia do [acesso à API JavaScript](https://formit.autodesk.com/page/formit-dynamo#dynamo-js-api-nodes). Obtenha-o [aqui](https://formit.autodesk.com/page/download).

### **Vários nós SendToFormIt**

O [FormIt 2021.3](https://formit.autodesk.com/blog/post/introducing-formit-2021-3) inclui a capacidade de usar [vários nós SendToFormIt e gráficos aninhados do Dynamo](https://formit.autodesk.com/page/formit-dynamo#dynamo-groups).

### **Nó SelectFromFormIt**

O [FormIt 2021](https://formit.autodesk.com/blog/post/introducing-formit-2021) inclui o nó [SelectFromFormIt](https://formit.autodesk.com/page/formit-dynamo#dynamo-formit-nodes) e permite sessões sempre conectadas, edição de várias instâncias e muito mais.
...
É a primeira vez que você usa o FormIt + Dynamo? Talvez seja necessário [configurar o sistema](https://formit.autodesk.com/page/formit-dynamo#dynamo-important-notes) primeiro para ver a tela 3D no Dynamo.
...
* Após inserir um grupo do Dynamo, selecione-o e alterne para o painel Propriedades ou simplesmente clique duas vezes no grupo para alternar automaticamente para Propriedades.
   ...
   * Os nós de entrada [**SelectFromFormIt**](https://formit.autodesk.com/page/formit-dynamo#dynamo-formit-nodes) são exibidos como botões na parte superior e podem ser usados para atualizar a seleção usada para controlar o gráfico.
...
* Clique com o botão direito do mouse na miniatura e clique em _ Editar gráfico incorporado._
...
* No Dynamo, você pode observar que o nome do arquivo na parte superior agora contém “(FormIt)”, o que significa que você está editando um gráfico que está incorporado neste arquivo do FormIt, e não modificando o Gráfico de origem.
* Certifique-se de que um ou mais nós [SendToFormIt](https://formit.autodesk.com/page/formit-dynamo#dynamo-formit-nodes) estejam conectados à geometria que você deseja enviar para o FormIt.
...
Os Gráficos de origem são exibidos no painel do Dynamo após a [vinculação dos diretórios locais](https://formit.autodesk.com/page/formit-dynamo#dynamo-getting-started). Esses gráficos são armazenados no computador e podem ser editados no Dynamo clicando no botão Editar gráfico de origem.

![](../.gitbook/assets/dynamo\_editsourcegraph.png)

![](../.gitbook/assets/dynamo\_sourcegraphgif.gif)

* [Vincule um diretório](https://formit.autodesk.com/page/formit-dynamo#dynamo-getting-started) que contenha arquivos do Dynamo ao painel do Dynamo e, em seguida, navegue para esse local no painel.&#x20;
...
* O Dynamo é iniciado com o gráfico solicitado aberto e, no FormIt, aparecerá a geometria da saída final do gráfico.
   * Para gráficos que usam um ou mais nós [SelectFromFormIt](https://formit.autodesk.com/page/formit-dynamo#dynamo-formit-nodes) como entrada, é possível que você não veja a geometria resultante até que os nós SelectFromFormIt sejam selecionados.
...
[Saiba mais sobre as configurações de facetamento de curva e superfície no FormIt.](https://windows.help.formit.autodesk.com/tool-library/curve-+-surface-faceting)
...
* Durante a seleção das geometria para um nó [SelectFromFormIt](https://formit.autodesk.com/page/formit-dynamo#dynamo-formit-nodes), é útil armazenar a geometria em um grupo do FormIt e selecionar o grupo.
...
* Quando a forma da construção é alterada, você pode simplesmente selecionar o grupo do sistema de montante e clicar em Executar no painel Propriedades.
...
* FormItGroupOptions é uma nova porta (opcional) no FormIt 2022 e é detalhada na seção **FormItGroupOptions Nodes**, abaixo.
...
* [Confira como o Dynamo funciona com grupos do FormIt](https://formit.autodesk.com/page/formit-dynamo#dynamo-groups).
...
* Após gerar a primeira vez, aparece um botão para cada nó SelectFromFormIt no painel Propriedades do FormIt.
...
* Nomeie o nó SelectFromFormIt para indicar o tipo de geometria que é esperado. Por exemplo, “Selecionar limite do terreno (arestas)”
   * É possível selecionar qualquer tipo de geometria do FormIt, mas frequentemente é melhor restringir a seleção em um grupo do FormIt e [selecioná-lo em vez da geometria original](https://formit.autodesk.com/page/formit-dynamo#dynamo-groups).
* Se você precisar mover os resultados de um gráfico do Dynamo com base na seleção, será melhor mover a geometria de seleção primeiro e, em seguida, executar novamente o gráfico, o que permitirá retomar a geometria de seleção atualizada e fazer o reposicionamento de forma apropriada.&#x20;
...
* Durante a seleção no FormIt, é aplicado o [Filtro de seleção](https://windows.help.formit.autodesk.com/tool-library/select-edge-face-or-object#selection-filtering) ativo. Por exemplo, se você desejar selecionar vértices do FormIt, será necessário ativá-los no Filtro de seleção.
...
No FormIt 2022.1.1 e versões mais recentes, se alternar um gráfico para usar os nós FormItLengthString (colocando o primeiro em um gráfico) ou alternar um gráfico para usar somente números brutos (removendo o último FormItLengthString), determinados comportamentos serão alterados ao editar um gráfico no Dynamo:

* Durante o uso do nó [SelectFromFormIt](https://formit.autodesk.com/page/formit-dynamo#dynamo-formit-nodes) ao editar um gráfico, a capacidade de alternar entre números brutos e o nó _FormItLengthString_, como acima, exige uma nova seleção da geometria para cada _SelectFromFormItNode_, para que os resultados continuem a ser dimensionados corretamente no FormIt.
* Após inserir o primeiro nó FormItLengthString em um gráfico, todos os números no gráfico que se destinam a cotas (incluindo entradas de número bruto) fazem referência a metros (unidade nativa do Dynamo).
   * O nó [SendToFormIt](https://formit.autodesk.com/page/formit-dynamo#dynamo-formit-nodes) considera a alteração e garante que a geometria gerada no FormIt permaneça com o tamanho correto.
...
* Para usar o Dynamo no FormIt, será necessário o [FormIt para Windows](https://formit.autodesk.com/page/download) v17.0 ou versão posterior.
...
* Se você tiver um sistema com uma [placa gráfica NVIDIA ou AMD](https://www.howtogeek.com/414201/how-to-check-what-graphics-card-gpu-is-in-your-pc/) ou várias placas, talvez seja necessário definir o FormIt e o Dynamo para usar a GPU de alta potência:
   ...
   * Use os painéis de controle [NVIDIA](https://nvidia.custhelp.com/app/answers/detail/a\_id/2615/\~/how-do-i-customize-optimus-profiles-and-settings%3F) ou [AMD](https://www.amd.com/en/support/kb/faq/dh-017) para definir os seguintes aplicativos para usar a placa gráfica independente:
...
Precisa de ajuda com o FormIt + Dynamo? [Nos informe nos fóruns](https://forums.autodesk.com/t5/formit-forum/bd-p/142).
