...
_V této kapitole budeme používat soubory ze složky_ **Farnsworth House Data Set > Supporting Files**. Pokud jste tak dosud neučinili, stáhněte si požadované složky nebo celou datovou sadu z _**datových sad k příručce Základy aplikace FormIt, Část 1**._
...
2 – Ve složce **Farnsworth House Data Set > Supporting Files > FormIt** v **datové sadě Farnsworth House** vytvořte novou složku s názvem _**Custom FormIt Content** (Vlastní obsah FormIt)._
...
2. Ve složce **Farnsworth House Data Set > Supporting Files > SketchUp** vyberte soubor **Ottoman – Barcelona\_Mies.skp** a klikněte na tlačítko **Otevřít**.
...
_Pokud si přejete, můžete svůj soubor porovnat s naším, a to tak, že otevřete soubor_ _**Ottoman - Barcelona\_Mies.axm**_ _uložený ve složce_ _**Farnsworth House Data Set > Supporting Files > FormIt > Furniture**_ _v_ _**datové sadě Farnsworth House**.‌_
...
3. V _**datové sadě Farnsworth House** přejděte do následujících složek:_ _**Supporting Files > FormIt**. Zde naleznete složky_ obsahující soubory **.axm**, které jsme vytvořili dříve v této kapitole. Dvojitým kliknutím vyberte složku **FormIt**.
...
_**Poznámka:**_ _Při přesouvání_ _**jádra** neměňte jeho výšku. Buď můžete pomocí klávesy_ _**Shift**_ _omezit pohyb vždy podél jedné z os, nebo můžete zajistit, aby počáteční i koncový referenční bod příkazu_ _**Přesunout (M)**_ _byly ve stejné výšce, a to kliknutím pouze na_ _**obrázek plánu**, nikoli na samotné_ _**jádro**_ _. Další informace o nástroji_ _**Přesunout (M)**_ _naleznete v předchozích kapitolách.‌_

## **Umístění nábytku z knihovny**

1 – Pomocí podobného postupu nyní můžete umístit nábytek, který jste vytvořili dříve v této kapitole, ze složky **Custom FormIt Content**. Pokud jste nepřevedli všechny tři (3) soubory SKP, můžete místo nich použít předpřipravené verze ve složce **Furniture**.

_**Poznámky:**_

* _Znovu zapněte hladinu_ _**Podlaží hlavní budovy**_ _, abyste mohli umístit nábytek přímo na povrch_ _**podlaží hlavní budovy**._
...
1 – Pomocí technik, které jste se právě naučili, umístěte jednu instanci komponenty **tree\_pine** ze složky **Farnsworth House Data Set > FormIt > Planting**.
